...
def filtrer_par_periode(df: pd.DataFrame, nb_annees: int = 5) -> pd.DataFrame:
    """Filtre les données pour garder seulement les nb_annees dernières."""
    if df.empty:
        return df
...
def ajouter_coordonnees(df: pd.DataFrame) -> pd.DataFrame:
    """Ajoute les coordonnées géographiques aux gares."""
    if df.empty:
        return df
    ...
    # Ajouter coordonnées départ
    if "gare_depart" in df.columns:
        df[["lat_depart", "lon_depart"]] = df["gare_depart"].apply(
            lambda x: pd.Series(trouver_coordonnees(x, gares_coords) or (None, None))
        )

    # Ajouter coordonnées arrivée
    if "gare_arrivee" in df.columns:
        df[["lat_arrivee", "lon_arrivee"]] = df["gare_arrivee"].apply(
            lambda x: pd.Series(trouver_coordonnees(x, gares_coords) or (None, None))
        )
